...
Color _typeColor(String type) => switch (type) {
      ...
      'bonus' => const Color(0xFFD97706),
      'promo' => const Color(0xFF0891B2),
...
Color _typeBg(String type) => switch (type) {
      ...
      'bonus' => const Color(0xFFFEF3C7),
      'promo' => const Color(0xFFE0F7FA),
...
class DriverNotificationScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<DriverNotificationScreen> createState() => _DriverNotificationScreenState();
}

class _DriverNotificationScreenState extends ConsumerState<DriverNotificationScreen>
    with SingleTickerProviderStateMixin {
  late final AnimationController _anim;
  String? _filter;
  List<_NotifItem> _all = [];
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _all = list.map(_NotifItem.fromJson).toList();
          _loading = false;
        });
        _anim..reset()..forward();
      }
  ...
  List<_NotifItem> get _filtered =>
      _filter == null ? _all : _all.where((n) => n.type == _filter).toList();
  ...
  Future<void> _markRead(_NotifItem item) async {
    ...
    try {
      await DioClient.create().patch('/users/notifications/${item.id}/read');
    } catch (_) {}
  }

  Future<void> _delete(_NotifItem item) async {
    setState(() => _all.remove(item));
    try {
      await DioClient.create().delete('/users/notifications/${item.id}');
    } catch (_) {}
  }
  ...
  Widget _buildFilterRow() {
    const chips = [
      ...
      ('ride', 'Order', Icons.motorcycle_rounded),
      ('payment', 'Pembayaran', Icons.account_balance_wallet_rounded),
      ('bonus', 'Bonus', Icons.emoji_events_rounded),
      ('info', 'Sistem', Icons.info_rounded),
      ('promo', 'Promo', Icons.local_offer_rounded),
    ];
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(20),
                border: Border.all(
                  color: selected ? AppColors.primaryColor : const Color(0xFFDDE3F5),
                ),
                boxShadow: selected
                    ? [const BoxShadow(color: Color(0x300540F2), blurRadius: 8, offset: Offset(0, 2))]
                    : null,
              ),
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(icon, size: 14, color: selected ? Colors.white : const Color(0xFF6B7BAD)),
                  const SizedBox(width: 5),
                  Text(label,
                      style: GoogleFonts.plusJakartaSans(
                          fontSize: 12,
                          fontWeight: FontWeight.w600,
                          color: selected ? Colors.white : const Color(0xFF6B7BAD))),
                ],
              ),
            ),
          );
      ...
      return Center(
        child: Column(
          ...
          children: [
            Container(
              width: 88, height: 88,
              decoration: const BoxDecoration(color: Color(0xFFE0E8FF), shape: BoxShape.circle),
            ...
            Text('Tidak ada notifikasi',
                style: GoogleFonts.plusJakartaSans(
                    fontWeight: FontWeight.w700, fontSize: 16, color: AppColors.primaryDark)),
